...
1. **Single tool.** The TS agent exposes `execute_code` only. No new tools.
2. **System prompt is primary.** All decision logic, gate order, and policy rules live in `agent/system-prompt.ts`. Hardcoding logic in the runner is a bug.
3. **Per-trial isolation.** Each BitGN trial gets its own `harnessUrl`. No shared state between concurrent tasks.
4. **verify() is mandatory.** Every `ws.answer()` must pass a `verify(scratchpad)` check. Never remove or weaken it.
5. **No card data in logs.** Payment card numbers must never appear in any output, log, or scratchpad field.
6. **No prod runs without dev validation.** Test on `bitgn/ecom-dev` before running `bitgn/ecom-prod`.

---

## Workflow Rules

- **Propose before implementing** any change that touches `agent/system-prompt.ts`, `agent/index.ts`, or `Dockerfile`. These are high-risk files.
...
- **Log first.** Before fixing a failing task, read its log in `runs/<timestamp>/<taskId>.jsonl` and the executed `.py` files.
